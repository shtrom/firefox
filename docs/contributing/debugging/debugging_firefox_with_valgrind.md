...
Linux: Download [Valgrind](https://valgrind.org/) directly, or use
your distribution's package manager (if it has a recent enough version).

MacOSX: [Get Valgrind trunk from
SVN](https://valgrind.org/downloads/repository.html) and build it.
...
[here](https://bugs.kde.org/show_bug.cgi?id=242137#c3) for an easy
...
Guide](http://www.valgrind.org/docs/manual/quick-start.html) is short
and worth reading. The [User
Manual](https://valgrind.org/docs/manual/manual.html) is also useful.
...
[Valgrind trunk from
SVN](http://www.valgrind.org/downloads/repository.html). Oftentimes
bugs are fixed in the trunk before most users fall across them. If that
doesn't help, consider [filing a bug
report](http://www.valgrind.org/support/bug_reports.html), and/or
