...
function fails with `PR_IO_TIMEOUT_ERROR` or
`PR_PENDING_INTERRUPT_ERROR`, the file descriptor must be closed
...
`PR_IO_TIMEOUT_ERROR`. If the thread gets interrupted by another
thread's `PR_Interrupt()` call, the IO function returns with
...
its `PRThread` structure. Then the thread is put to sleep. In the
...
- if the overlapped IO request is not completed before the specified
  ...
  case, the error is `PR_IO_TIMEOUT_ERROR`.
- if the thread gets interrupted by another thread's
  `PR_Interrupt()` call. In this case, the error is
  `PR_PENDING_INTERRUPT_ERROR`.
...
`PRThread` structure. If the thread subsequently exists and its
`PRThread` structure gets deleted, the pointer to the overlapped
...
before the thread exits. NT's `CancelIo()` function would be
ideal for this purpose. Unfortunately, `CancelIo()` is not
...
- Once there is a timeout or interrupt error, the file descriptor is no
  ...
  `PR_DisableInterrupt()` function to turn off interrupts when
  accessing such file descriptors.

%

*A related known bug is that timeout and interrupt don't work for
`PR_Connect()` on NT. This bug is due to a different
...
`CancelIo()` function to cancel outstanding overlapped IO
requests when we get IO timeout or interrupt errors. If
`CancelIo()` really works as advertised, that should
...
- Author: <mailto:larryh@netscape.com>
