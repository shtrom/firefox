...
class MediaSessionTest : BaseSessionTest() {
    ...
    // End-to-end on a real page: a system audio-focus loss routed through
    // notifySystemAudioFocusChange drives the Audio Session interrupt and
    // suspends an audible media element; a subsequent gain revives what a
    // transient loss suspended. Verified via the actual playback state (the
    // MediaSession.Delegate), not a mock. The element loops so it cannot end
    // (and pause on its own) before the interruption is driven. Web Audio and
    // Web Speech reach the same chrome interrupt but are uncontrollable (no
    // onPlay/onPause), and their suspend/resume is covered by
    // dom/media/mediacontrol/tests/browser/browser_audio_focus_interrupt_suspend_resume.js.
    private fun checkMediaElementInterrupt(lossReason: Int, resumesOnGain: Boolean) {
        sessionRule.setPrefsUntilTestEnd(
            mapOf(
                "dom.audio_session.enabled" to true,
                "media.autoplay.default" to 0,
        ...
        val onPlayCalled = arrayOf(GeckoResult<Void>(), GeckoResult<Void>())
        val onPauseCalled = GeckoResult<Void>()
        var mediaSession: MediaSession? = null

        val session = sessionRule.createOpenSession()
        session.delegateUntilTestEnd(object : MediaSession.Delegate {
            override fun onActivated(session: GeckoSession, ms: MediaSession) {
                mediaSession = ms
                onActivatedCalled.complete(null)
            }

            override fun onPlay(session: GeckoSession, ms: MediaSession) {
                val index = sessionRule.currentCall.counter - 1
                if (index < onPlayCalled.size) {
                    onPlayCalled[index].complete(null)
                }
            }

            override fun onPause(session: GeckoSession, ms: MediaSession) {
                onPauseCalled.complete(null)
            }
        })

        session.loadTestPath(MEDIA_SESSION_AUDIO_LOOP_PATH)

        // The element autoplays, activating the session and starting playback.
        sessionRule.waitForResult(
            GeckoResult.allOf(onActivatedCalled, onPlayCalled[0]),
        )

        mediaSession!!.notifySystemAudioFocusChange(lossReason)
        sessionRule.waitForResult(onPauseCalled)

        if (resumesOnGain) {
            mediaSession.notifySystemAudioFocusChange(
                MediaSession.SYSTEM_AUDIO_FOCUS_GAIN,
            )
            sessionRule.waitForResult(onPlayCalled[1])
        }
    }

    @Test
    fun mediaElementSuspendsAndResumesOnTransientAudioFocusLoss() {
        checkMediaElementInterrupt(
            MediaSession.SYSTEM_AUDIO_FOCUS_TRANSIENT_LOSS,
            resumesOnGain = true,
        )
    }

    @Test
    fun mediaElementSuspendsOnPermanentAudioFocusLoss() {
        checkMediaElementInterrupt(
            MediaSession.SYSTEM_AUDIO_FOCUS_PERMANENT_LOSS,
            resumesOnGain = false,
        )
    }
