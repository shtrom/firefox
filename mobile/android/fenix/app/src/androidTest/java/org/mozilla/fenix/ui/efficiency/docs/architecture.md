...
How the framework is built and *why*. Read this once to get the mental model; the `guides/` cover the
...
- **`core/`** — the element plumbing. The `resolve()` seam turns a `Selector` into an on-screen element.
  ...
  *displayed* match (this is why Compose "merged-vs-unmerged" bugs don't recur), and it degrades to `false`
