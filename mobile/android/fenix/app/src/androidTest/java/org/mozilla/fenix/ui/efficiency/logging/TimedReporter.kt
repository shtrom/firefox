 ...
 * - Optionally forwards structured step events into the existing factory logging pipeline (StepLogger), so we can
 *   retain artifacts like summary.log / details.jsonl while iterating on console UX.
 *
 ...
 *     - This is particularly important once tests are dynamically generated by factories, CI config, or
 *       reflection-based enumeration of pages/components.
 *
 ...
 *     - Once we can generate test permutations (pages x states), we need a reliable way to *see* what actually ran. The
 *       log stream becomes a ground truth record.
...
class TimedReporter(private val forwarder: StepLogger? = null) {
    ...
    /**
     * STEP: highest-level intent (e.g., "Navigate to BookmarksPage"). CMD : custom command / helper (e.g., "click menu
     * item", "verify page loads"). LOC : element lookup / verification.
     */
    enum class Type {
        STEP,
        CMD,
     ...
     *
     * Future direction:
     * - Allow overrides by system properties so CI can tune thresholds per device tier. e.g. -DslowLocMs=400,
     *   -DslowCmdMs=1200
     */
     ...
     * - "Wall time" is the overall elapsed real-world time (start -> end) and includes uninstrumented work
     *   (setup/teardown, system scheduling, sleeps, etc.).
     */
    data class Totals(
        var steps: Int = 0,
        var cmds: Int = 0,
    ...
    private fun endEvent(success: Boolean, message: String) {
        ...
        val completionText = (message.takeIf { it.isNotBlank() } ?: (if (success) "OK" else "FAILED")) + warnText
    ...
    fun printSummary() {
        ...
        ConsoleLogger.info(
            1,
            "Steps: ${totals.steps} (total ${"%.1f".format(totals.stepMs)} ms, avg ${"%.1f".format(stepAvg)} ms)",
        )
        ConsoleLogger.info(
            1,
            "Commands: ${totals.cmds} (total ${"%.1f".format(totals.cmdMs)} ms, avg ${"%.1f".format(cmdAvg)} ms)",
        )
        ConsoleLogger.info(
            1,
            "Locators: ${totals.locs} (total ${"%.1f".format(totals.locMs)} ms, avg ${"%.1f".format(locAvg)} ms)",
        )
