...
- **Slow ≠ flaky (E1).** UIAutomator list rendering can take ~3s; `mozVerify` polls-until-present (logs
  "not found" retries, then finds it). If it passed on try #1 with retries *inside* the poll, that's fine —
  not flaky.
- **Retry can mask a real red (A5, MTE-5729).** `BaseTest.isRetryable()` is broad; one retry can turn a
  genuine failure green. A test that only passes on a *test-level* retry is flaky/failing, not done.
...
- **A shared-resolution change breaks many tests uniformly (A4).** Editing `resolve()` / a shared selector
  touches every test that uses it. Re-run the WHOLE class (or suite) after any shared change; a *uniform*
  mass failure = systematic selector break, *scattered* failures = flakiness. Don't re-tune shared
