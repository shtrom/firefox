...
Native Profiling
----------------
To generate CI profiles with an OS-specific profiler (i.e., Simpleperf for Android, Xperf for Windows), run ``./mach try`` and select any job with the ``-native-profiling`` suffix (i.e. a `supported job <browsertime.html#native-profiling-with-raptor-browsertime>`_).
