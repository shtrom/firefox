...
promise_test(async t => {
  ...
  const encoder = new VideoEncoder({
    output(chunk, meta) {
      encodedChunk = chunk;
      if (meta.decoderConfig) encoderConfig = meta.decoderConfig;
    },
    error(e) { t.unreached_func('Encoder error: ' + e)(); },
  });

  encoder.configure({codec: 'vp8', width, height, bitrate: 5_000_000});
  encoder.encode(inputFrame);
  inputFrame.close();
  await encoder.flush();
  assert_not_equals(encodedChunk, null, 'Got encoded chunk');
  assert_not_equals(encoderConfig, null, 'Got decoder config');

  decoder.configure(encoderConfig);
