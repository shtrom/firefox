...
/**
 * A scheduled pass rotation whose timer fires while suspended
 * (asleep/backgrounded) is skipped, since the network is down. The wake handler
 * is what requests the rotation once we're back awake.
 */
add_task(async function test_scheduled_rotation_skipped_while_suspended() {
  let sandbox = sinon.createSandbox();
  setupStubs({ validProxyPass: true });

  // Activate with a pass that is due for rotation almost immediately, so the
  // manager schedules a rotation that fires shortly after activation.
  const now = Temporal.Now.instant();
  const dueForRotation = new ProxyPass(
  ...
  // Go to sleep before the scheduled rotation timer fires.
  Services.obs.notifyObservers(null, SLEEP_TOPIC);

  // The scheduled rotation reads isSuspended when its timer fires. Resolve on
  // that read so we wait for the actual callback rather than a fixed delay.
  const guardChecked = Promise.withResolvers();
  let isSuspendedChecked = false;
  sandbox.stub(IPPLifecycleHelper, "isSuspended").get(() => {
    isSuspendedChecked = true;
    guardChecked.resolve();
    return true;
  });

  await guardChecked.promise;

  Assert.ok(isSuspendedChecked, "The scheduled rotation checked isSuspended");
  Assert.ok(
    rotateSpy.notCalled,
    "The scheduled rotation is skipped while suspended"
  );
