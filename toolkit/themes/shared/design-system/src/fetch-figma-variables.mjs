...
// Selection flags forwarded from `mach buildtokens` after `--`. `--match=<substr>`
// (repeatable) restricts the import to changed tokens whose path contains the
// substring; `--all` imports every change without prompting. With neither flag
// we open an editor to review the changed tokens (unless stdin isn't a TTY).
const matchFilters = [];
let importAll = false;
...
// `figma-variables-all.json` is a faithful mirror of the Figma export, so we
// write the freshly fetched result wholesale (reordered to match the previous
// file purely to keep the diff small). Nothing is merged into it per token; the
// curated subset the build actually reads lives in
// `nova-export-clean-variables.json`.
const ordered = reorderToMatch(result, existing);
writeFileSync(outputPath, JSON.stringify(ordered, null, 2) + "\n");

// The changed tokens are computed from the Nova override values a Figma import
// would produce, not from the raw export. This hides changes that don't affect
// the overrides (e.g. a Figma value that still resolves to the base default),
// and keys them by their Nova path (`button/background/color`) rather than by
// collection and mode.
const oldNova = computeNovaValues(existing);
...
if (!changed.length) {
  // eslint-disable-next-line no-console
  console.log(`No token changes to import. Wrote ${outputPath}`);
  process.exit(0);
}
...
// from every group and re-copy them from the fresh export. A resolved Nova path
// can live in more than one group (e.g. Primitives and Components); we mirror it
...
// Order the subset to match the mirror (`ordered`), not the raw REST response
// (`result`) whose key order differs from how the file is stored. Otherwise
// every token reshuffles on each fetch, burying the real value changes.
const orderedImported = reorderToMatch(imported, ordered);
writeFileSync(importedPath, JSON.stringify(orderedImported, null, 2) + "\n");

// eslint-disable-next-line no-console
console.log(
  `Applied ${selected.length} of ${changed.length} changed tokens. ` +
    `Wrote ${outputPath} and ${importedPath}`
);
