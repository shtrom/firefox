...
nsresult nsHostResolver::ResolveHost(const nsACString& aHost,
  ...
  nsresult status = NS_OK, rv = NS_OK;
  {
    ...
    if (IS_ADDR_TYPE(type) && IsLoopbackHostname(host)) {
      ...
      // Don't reuse a cached negative when both families are negative; force a
      // refresh so a transient DNS failure doesn't stick across reloads.
...
bool nsHostResolver::OtherFamilyHasUsablePositiveResult(
  ...
  if (aAf == PR_AF_INET) {
    ...
    // For an AF_UNSPEC lookup a negative result already means both families
    // failed, so there is no other family to fall back on.
    return false;
  }
