...
The case studies below are snapshots of past investigations, not a command reference: they predate some commands the guide documents, so treat a walkthrough's commands as one route rather than the full set, and use the guide output for the full list of commands. Use `profiler-cli status` to confirm the selected thread, zoom and filters before drawing conclusions from a filtered view.
