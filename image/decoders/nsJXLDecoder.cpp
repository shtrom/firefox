...
void nsJXLDecoder::BuildCMSTransform() {
  // Prefer building the input profile directly from CICP enum values when the
  // input color space maps to standard CICP values.
  ...
  // Fall back to the ICC profile for everything CICP didn't handle: PQ/HLG,
  // grayscale, CMYK, embedded ICC, custom color points, and CICP curves qcms
  // doesn't implement (qcms_profile_create_cicp returns null above).
  if (!mInProfile) {
    size_t iccLen = 0;
