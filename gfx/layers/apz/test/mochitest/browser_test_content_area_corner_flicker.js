...
// See browser_test_position_sticky.js for why this is needed.
...
add_task(async () => {
  ...
  const url = httpURL("helper_content_area_corner_flicker.html");
  ...
  // Compare only the top strip of the sticky band, which spans both of the
  // content area's rounded top corners. The strip height is a fixed value
  // (independent of the corner radius) so that the comparison still exercises a
  // real region when there is no rounding, and comfortably exceeds the largest
  // content-area corner radius. See also Bug 2061338 for rounded corner rendering
  // issue.
  rect.height = 20;
  ...
  // Take a snapshot again while scrolling; the corner strip should be unchanged.
