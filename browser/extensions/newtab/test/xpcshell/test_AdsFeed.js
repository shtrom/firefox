...
add_task(async function test_fetchWithAdsClient_refreshesContextId() {
  info(
    "AdsFeed: _fetchWithAdsClient() should refresh the browser-wide context id " +
      "from ContextId.request() and push it to AdsClient before issuing MAC ad requests"
  );
  ...
  const TEST_CONTEXT_ID = "ctx-NEW";
  const reqStub = sandbox.stub(ContextId, "request").resolves(TEST_CONTEXT_ID);
  const updateStub = sandbox.stub(AdsClient, "updateContextId");
  const tileFake = sinon.fake.resolves(new Map());
  const spocFake = sinon.fake.resolves(new Map());

  sandbox.stub(AdsClient, "isEnabled").returns(true);
  sandbox.stub(AdsClient, "getClient").returns({
    requestTileAds: tileFake,
    requestSpocAds: spocFake,
  });
  sandbox.stub(AdsClient, "requestOptions").returns({
    flags: new Map(),
    ohttp: false,
  });

  const feed = getAdsFeedForTest();
  feed.adsClient = AdsClient.getClient();

  await feed._fetchWithAdsClient(
    { tiles: true, spocs: true },
    [
      { placement: "newtab_tile_1", count: 1 },
      { placement: "newtab_spocs", count: 6 },
  ...
  Assert.ok(
    updateStub.calledWith(TEST_CONTEXT_ID),
    `AdsFeed should push the freshly resolved context id ('${TEST_CONTEXT_ID}') into AdsClient.updateContextId()`
  );
  Assert.ok(
    reqStub.calledBefore(tileFake),
    "ContextId.request() should be awaited before AdsClient.requestTileAds() so the cached id is never stale"
  );
  Assert.ok(
    updateStub.calledBefore(tileFake),
    "AdsClient.updateContextId() should run before requestTileAds() so the provider reads the fresh id, not a stale one"
  );
...
add_task(
  async function test_fetchWithAdsClient_refreshesContextIdOnEveryFetch() {
    info(
      "AdsFeed: _fetchWithAdsClient() should re-read the context id from " +
        "ContextId.request() on every fetch, so a rotated id is never served stale"
    );
    let sandbox = sinon.createSandbox();
    sandbox.stub(AdsFeed.prototype, "PersistentCache").returns({
      set: () => {},
    ...
    const TEST_CONTEXT_ID = "ctx-1";
    const TEST_CONTEXT_ID_ROTATED = "ctx-2-rotated";
    const reqStub = sandbox.stub(ContextId, "request");
    reqStub.onFirstCall().resolves(TEST_CONTEXT_ID);
    reqStub.onSecondCall().resolves(TEST_CONTEXT_ID_ROTATED);
    const updateStub = sandbox.stub(AdsClient, "updateContextId");
    sandbox.stub(AdsClient, "isEnabled").returns(true);
    sandbox.stub(AdsClient, "getClient").returns({
      requestTileAds: sinon.fake.resolves(new Map()),
      requestSpocAds: sinon.fake.resolves(new Map()),
    });
    sandbox.stub(AdsClient, "requestOptions").returns({
      flags: new Map(),
      ohttp: false,
    ...
    await feed._fetchWithAdsClient(...fetchArgs);
    await feed._fetchWithAdsClient(...fetchArgs);

    Assert.equal(
      updateStub.callCount,
      2,
      "context id cache is refreshed once per fetch, never reused across fetches"
    );
    Assert.ok(
      updateStub.getCall(0).calledWith(TEST_CONTEXT_ID),
      "first fetch caches the current context id"
    );
    Assert.ok(
      updateStub.getCall(1).calledWith(TEST_CONTEXT_ID_ROTATED),
      "second fetch caches the rotated context id, not a stale cached value"
    );
