 ...
 * all of which only exist in Firefox 154+. When newtab train-hops onto an older
 * host these are absent (the element never upgrades, the actor is unregistered,
 * and the ftl `<link>` fails silently), so gate the whole feature off there.
 * Remove this guard once 154 reaches Release.
 */
function isThemePickerHostSupported() {
  return Services.vc.compare(AppConstants.MOZ_APP_VERSION, "154.0a1") >= 0;
}
const PREF_DEFAULTS = [
