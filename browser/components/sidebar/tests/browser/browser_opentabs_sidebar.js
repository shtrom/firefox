...
add_task(async function test_search_filters_visible_rows() {
  ...
  // A query switches to the flat results view and filters to matches.
  ...
  const resultsList = getTabList(component);
  await waitForRowCount(resultsList, 1);
  Assert.ok(
    resultsList.rowEls[0].url.includes("Apple"),
    "The one matching tab is shown in the results list."
  );
  ...
  // Clearing the query returns to the per-window view with all rows.
  searchBox.dispatchEvent(
  ...
  await BrowserTestUtils.waitForMutationCondition(
    component.shadowRoot,
    { childList: true, subtree: true },
    () => component.shadowRoot.querySelector(".window-card")
  );
  await waitForRowCount(getTabList(component), allTabsCount);
  Assert.equal(
