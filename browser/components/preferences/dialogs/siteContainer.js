 ...
 * container. A pasted URL is accepted and reduced to its host.
 *
 * @param {string} value
 * @returns {?string}
 */
function siteFromInput(value) {
  ...
  if (site.includes("://")) {
    try {
      site = Services.io.newURI(site).host;
    } catch (e) {
      return null;
    }
  }
...
function buildForm() {
  ...
  document
    ...
    .append(gSiteInput, gContainerSelect);
}
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  let updateValidity = () => {
    acceptButton.disabled = !siteFromInput(gSiteInput.value);
  };
  gSiteInput.addEventListener("input", updateValidity);
  updateValidity();

  document.addEventListener("dialogaccept", () => {
    ContextualIdentityService.setSiteAssociation(
      siteFromInput(gSiteInput.value),
