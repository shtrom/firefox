...
// Bug 2043582: with a prior search active, the user selects all the search
// text and retypes a different term one character at a time. Setting-groups
// hidden by earlier keystrokes used to stay stuck with `visually-hidden`.
...
/**
 * Counts SELECTION_FIND highlight ranges on the top-level document only.
 * Shadow-root documents use separate selection controllers, so their ranges
 * are excluded.
 */
...
/**
 * Waits until findRangeCount is unchanged across two animation frames. A stale
 * search can re-add ranges through late l10n awaits, so we wait for that search
...
// Bug 2045266: overlapping searchFunction calls must not leave duplicate
// SELECTION_FIND ranges compared to a fresh search.
add_task(async function overlapping_searches_produce_correct_range_count() {
  ...
  // Fire two searches back to back without awaiting the first. The second call
  // aborts the first, so the stale search resumes and returns at the abort check
  // after gotoPref. This exercises the top-level path, not the deeper race inside
  // searchWithinNode.
...
// Bug 2045266: same overlap as above, driven through real searchInput
// keystrokes instead of calling searchFunction directly. Headless collapses a
// single sendString into one search, so the query is typed in two bursts. To
// recreate the intermittent race a user can trigger by typing quickly, a
// gotoPref hook blocks the stale search until the later one finishes.
add_task(async function str_faithful_select_and_retype_range_count() {
  const STALE_QUERY = "do";
  const TARGET_QUERY = "downloads";
  ...
  // A completed search must already be present for the retype to overlap it.
  doc = await openSearchWithQuery("tabs");
  let win = gBrowser.contentWindow;
  let searchInput = doc.getElementById("searchInput");

  // Block the stale search at its first gotoPref await until the later search
  // finishes, then release it so the following abort check cancels it.
  let releaseStale;
  ...
  try {
    // STR: select the existing query, then retype. First burst snapshots the
    // distinct stale query and blocks its search at the first gotoPref await.
    searchInput.focus();
    searchInput.select();
    ...
    Assert.equal(
      ...
      freshCount,
      "Select-and-retype overlap must not inflate the find-selection range count"
    );
  } finally {
