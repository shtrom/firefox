...
class TestNewTabOnRestore(SessionStoreTestCase):
    """
    Test that a new tab is opened after session restore when the
    newTabOnRestore feature is enabled via real startup.
    """
    ...
    def test_new_tab_opens_when_enabled(self):
        ...
        self._quit_and_restore(expected_tab_count=3)
    ...
    def test_no_new_tab_when_disabled(self):
        ...
        self._quit_and_restore(expected_tab_count=2)
    ...
    def test_reuses_existing_newtab_rightmost(self):
        ...
        # Add a new tab as the last tab so it's part of the session
    ...
    def test_no_new_tab_when_cmdline_url(self):
        ...
        try:
            ...
            Wait(self.marionette, timeout=5, interval=0.1).until(
                lambda _: self._get_tab_count() >= 2,
                message="Expected at least 2 tabs after restore",
            )
    ...
    def test_reuse_newtab_selects_visible_tab(self):
        ...
        # Add a visible newtab, then a hidden newtab after it
        self.marionette.execute_script(
            """
            let win = BrowserWindowTracker.getTopWindow();
            let { AboutNewTab } = ChromeUtils.importESModule(
        ...
        # 2 original + 1 visible newtab + 1 hidden newtab = 4
        self._quit_and_restore(expected_tab_count=4)
