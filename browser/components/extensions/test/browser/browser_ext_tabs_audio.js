...
add_task(async function () {
  ...
  extension.onMessage("change-tab", async (tabId, attr, on) => {
    ...
    if (attr == "muted") {
      ...
      // Play or stop a real (Web Audio) sound so the tab's audibility — and
      // thus the soundplaying indicator the tabs API reports as `audible` —
      // follows the genuine media-control path rather than being toggled
      // artificially through tabbrowser internals.
