...
/**
 * @typedef {object} FieldData Field data for LLM classification request
 * @property {string} id Field id to map values to
 * @property {string} label Label text associated with the field
 * @property {string} name The element name attribute. It may be descriptive or opaque
 ...
 * @property {string} autocomplete The HTML autocomplete attribute, when present
 * @property {number | null} maxlength Maximum allowed character count, null when not
 * present
 * @property {Array<SelectOptionFormData>} options Dropdown choices, empty for non-select controls.
 * Each choice contains a frontend-assigned id and visible label.
 * @property {string} textBefore Nearest visible text immediately preceding the
 * current dom element to be filled.
 * @property {string} textAfter Bounded visible text immediately after the field
...
/**
 * @typedef {object} FieldDataForValueGen Field data for LLM value gen request
 * @property {string} id Field id to map values to
 * @property {string} label Label text associated with the field
 * @property {string} name The element name attribute. It may be descriptive or
 * opaque
 * @property {string} inputType Control type such as text, email, textarea,
 * select
 * @property {string} placeholder Placeholder text shown inside the control
 ...
 * current dom element to be filled.
 * @property {string} textAfter Bounded visible text immediately after the field
 * @property {string | null} type The classified type from local/llm passes
 * @property {"low" | "medium" | "high" | null} classificationConfidence Confidence score from local/llm model passes
 */
 ...
 * @property {Array<FieldData>} fields List of fields to classify
 */

/**
 * @typedef {object} RelevantTab
 * @property {string} id The stable tab ID
 * @property {"high" | "medium" | "low"} relevance Level of relevance
 ...
 * @property {Array<FieldData>} fields Form fields used to determine relevance
 */
 ...
 * @property {PageInfo} page Info about the page the form is on
 * @property {Array<FieldDataForValueGen>} fields List of fields to classify
 ...
 * @property {string} id The stable field ID
 * @property {"fill_from_token" | "select_option" | "generate"} action The action the LLM decided for the value
 * @property {string} [token] The token name when action is "fill_from_token"
 * @property {"high" | "medium" | "low"} confidence The LLM's value confidence
 * @property {string} [optionId] Select option stable ID when action is
 * "select_option"
 * @property {string} [value] The generate value when action is "generate"
 */
...
async function generateFormValuesBatch(request, { signal } = {}) {
  ...
  // TODO: Remove this after implementing LLM requests
  const fields = request.fields.map(field => ({
    id: field.id,
    action: "generate",
    value: mockValues[field.inputType] ?? "test",
    confidence: "high",
  }));
