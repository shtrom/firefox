...
// What the mocked model returns. Block coverage is browser_aitab_validation.js'
...
const GENERATED_PAGE = Object.freeze({
  ...
  blocks: [{ type: "info", heading: "Cheapest", value: "$72 / night" }],
});
...
add_task(async function test_generateAITab_success() {
  ...
  try {
    ...
    Assert.equal(
      result.metadata.themeId,
      GENERATED_PAGE.theme,
      "themeId comes from the page theme"
    );
    Assert.deepEqual(
...
add_task(async function test_generateAITab_theme_defaults_to_firefox() {
  const mockEngine = new MockEngineManager();
  const { url, cleanup: stopServing } = servePage();
  try {
    const genPromise = generateAITab({ urlList: [url] }, newConversation());

    // theme is optional in the page schema, which declares "firefox" as its
    // default, so an omitted theme must not surface as an unresolvable id.
    const { theme, ...pageWithoutTheme } = GENERATED_PAGE;
    Assert.equal(theme, "firefox", "the fixture carries an explicit theme");
    await mockEngine.respondTo({
      purpose: MODEL_FEATURES.AITAB,
      response: JSON.stringify(pageWithoutTheme),
    });
    ...
    Assert.equal(
      result.metadata.themeId,
      "firefox",
      "themeId falls back to the schema's default theme"
    );
  } finally {
    await stopServing();
    mockEngine.cleanupMocks();
