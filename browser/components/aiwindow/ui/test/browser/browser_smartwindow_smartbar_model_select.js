 ...
 * selected-model checkmark, then closes the panel again. Only meaningful with
 * the Mistral release pref on, since the checkmark is gated behind it.
 *
 * @param {MozBrowser} browser - The browser element
    ...
    for (const item of panelList.querySelectorAll("button.model-item")) {
      if (item.querySelector(".model-item-check")) {
        const iconSrc = item
          ...
          .getAttribute("src");
        checkedIds.push(iconSrc.match(/model-choice-(\d+)-v2\.svg/)?.[1]);
      }
    }
...
add_task(
  async function test_smartbar_model_select_check_marks_selected_model() {
    ...
    await SpecialPowers.pushPrefEnv({
      set: [["browser.smartwindow.mistralRelease", true]],
    });
    ...
    try {
      ...
      // A fresh tab starts on the default model, so the check should sit there.
      Assert.deepEqual(
        await getCheckedModelChoiceIds(browser),
        [DEFAULT_MODEL_CHOICE_ID],
        "Only the selected (default) model item should show the checkmark"
      );

      // Overriding the model should move the check to the new selection, proving
      // the check tracks selectedModelId rather than the default choice.
