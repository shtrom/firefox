...
add_task(async function test_sidebar_splitter_clamps_at_max_width() {
  try {
    AIWindowUI.openSidebar(window, null);
    await waitForSidebarOpen(window);

    const { box, splitter } = AIWindowUI._getSidebarElements(window);
    const sidebarMaxWidth = parseInt(window.getComputedStyle(box).maxWidth);

    const maxDragDistance = -1 * window.innerWidth;
    EventUtils.synthesizeMouseAtCenter(splitter, { type: "mousedown" });
    // Attempt to overdrag by 1px.
    EventUtils.synthesizeMouse(splitter, maxDragDistance - 1, 0, {
      type: "mousemove",
    });
...
add_task(async function test_sidebar_splitter_does_not_drag_over_devtools() {
  ...
  try {
    ...
    const browserStack = gBrowser.getPanel().querySelector(".browserStack");
    const contentAreaMin = parseInt(
      window.getComputedStyle(browserStack).minWidth
    );

    const { splitter } = AIWindowUI._getSidebarElements(window);
    const maxDragDistance = -1 * window.innerWidth;
    EventUtils.synthesizeMouseAtCenter(splitter, { type: "mousedown" });
    // Attempt to overdrag by 1px.
    EventUtils.synthesizeMouse(splitter, maxDragDistance - 1, 0, {
