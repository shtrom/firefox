...
const CAPTURE_PREF = "gfx.webrender.debug.enable-capture";

add_task(async function test_capture_shortcuts_are_pref_gated() {
  // This test covers the default configuration, in which WebRender captures are
  // disabled. Builds configured with --enable-webrender-debugger enable them.
  let capturesEnabled = Services.prefs.getBoolPref(CAPTURE_PREF);
  ok(!capturesEnabled, "WebRender captures are disabled by default.");
  if (capturesEnabled) {
    return;
  }
  ...
  ok(
    !ShortcutUtils.isSystem(window, "Ctrl+Shift+3"),
    "Ctrl+Shift+3 is available for other shortcuts."
  );
  ok(
    !ShortcutUtils.isSystem(window, "Ctrl+Shift+6"),
...
add_task(async function test_capture_shortcut_triggers_a_capture() {
  let captured = Promise.withResolvers();
  let webrenderCapture = gGfxUtils.webrenderCapture;
  gGfxUtils.webrenderCapture = () => captured.resolve();
  registerCleanupFunction(() => {
  ...
  ok(true, "The capture shortcut triggered a capture.");
});
