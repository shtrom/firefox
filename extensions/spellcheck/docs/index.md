...
[SCOWL] dataset. This document describes the process to add new words to the
...
[the Ubuntu man page for hunspell](https://manpages.ubuntu.com/manpages/bionic/man5/hunspell.5.html).
...
[this bug](https://bugzilla.mozilla.org/show_bug.cgi?id=enus-dictionary):
...
new bug][file a new bug] and move the alias `enus-dictionary` (in the *Details* section)
...
3. Identify the current version of SCOWL by checking the file
...
4. Clone the upstream SCOWL repository into the working directory and check out
...
5. There’s a special script used for editing dictionaries. The script
...
6. Run the script `sh make-new-dict.sh` to generate a new dictionary and make
   sure it runs without errors. For more details on this script, see the
   [make-new-dict.sh] section.

7. Run `sh verify-new-dict.sh` to sanity-check the regenerated dictionary
   (see the [verify-new-dict.sh] section). The script must report
   `Errors: 0` before proceeding.

8. If everything looks correct, use `sh install-new-dict.sh` to copy the
   generated file in the right position.

9. Build Firefox and test your updated dictionary. Once you’re
   satisfied, use the process described in {ref}`write-a-patch` to create a
   patch.
...
The English dictionary available in the [firefox repository] is based on the
[SCOWL] dictionary. Some scripts distributed with the SCOWL package are
...
1. Clone the upstream SCOWL repository into the working directory and check out
   ...
   SCOWLv2 requires Python 3.7+ and SQLite 3.33+.
2. Run the script `sh make-new-dict.sh` to generate a new dictionary and make
   sure it runs without errors. For more details on this script, see the
   [make-new-dict.sh] section.
3. Run `sh verify-new-dict.sh` to sanity-check the regenerated dictionary
   (see the [verify-new-dict.sh] section). The script must report
   `Errors: 0` before proceeding.
4. If everything looks correct, use `sh install-new-dict.sh` to copy the
   generated file in the right position and use the process described in
   {ref}`write-a-patch` to create a patch.
...
- That the upstream `en_US.txt` from the [wordlist-diff] mirror (fetched
...
[bug 237921]: https://bugzilla.mozilla.org/show_bug.cgi?id=237921
[file a new bug]: https://bugzilla.mozilla.org/show_bug.cgi?id=enus-dictionary
[scowl]: http://wordlist.aspell.net
[firefox repository]: https://github.com/mozilla-firefox/firefox
[wordlist-diff]: https://github.com/en-wl/wordlist-diff
