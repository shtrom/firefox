...
NS_IMETHODIMP TextInputProcessorListener::OnNotify(
  ...
  nsAutoCString type;
  aNotification->GetType(type);
  *aRetVal = true;
  if (type.EqualsLiteral("request-to-commit")) {
    nsresult rv = aTextInputProcessor->CommitComposition(nullptr, 0, 0);
    if (NS_FAILED(rv)) {
      *aRetVal = false;
      return NS_OK;
    }
  } else if (type.EqualsLiteral("request-to-cancel")) {
    nsresult rv = aTextInputProcessor->CancelComposition(nullptr, 0, 0);
    if (NS_FAILED(rv)) {
      *aRetVal = false;
      return NS_OK;
    }
